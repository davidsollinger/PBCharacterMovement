...
UPBPlayerMovement::UPBPlayerMovement()
{
	...
	BrakingWindow = 15.f;
	// Crouching
	Super::SetCrouchedHalfHeight(34.29f);
...
void UPBPlayerMovement::DoCrouchResize(float TargetTime, float DeltaTime, bool bClientSimulation)
{
	...
	if (FMath::IsNearlyEqual(CharacterCapsule->GetUnscaledCapsuleHalfHeight(), Super::GetCrouchedHalfHeight()))
	{
		if (!bClientSimulation)
		{
	...
	const float crouchHalfHeight = Super::GetCrouchedHalfHeight();
	...
	const float FullCrouchDiff = OldUnscaledHalfHeight - crouchHalfHeight;
	...
	const float CurrentAlpha = 1.0f - (CurrentUnscaledHalfHeight - crouchHalfHeight) / FullCrouchDiff;
...
void UPBPlayerMovement::DoUnCrouchResize(float TargetTime, float DeltaTime, bool bClientSimulation)
{
	...
	const float crouchedHalfHeight = Super::GetCrouchedHalfHeight();
	const float ComponentScale = CharacterCapsule->GetShapeScale();
